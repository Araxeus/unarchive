...
const redBackground = (str) => `\x1b[41m${str}\x1b[0m`;
const greenBackground = (str) => `\x1b[42m${str}\x1b[0m`;
...
const fixturesPaths = fixtures.map((p) => path.join(process.cwd(), 'tests', 'fixtures', p));
...
const destPaths = fixtures.map((p) => path.join(dest, `${removeExtension(p)}.txt`));
for (const destPath of destPaths) {
    const file = await readFile(destPath, 'utf-8');
    const expected = `hello from ${path.basename(destPath, '.txt')}`;
    if (file !== expected) {
        throw new Error(`File ${destPath} content is not correct\nExpected:\n${greenBackground(expected)}\nActual:\n${redBackground(file)}`);
    }
}
